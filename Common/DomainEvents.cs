...
namespace ddd_tutorial_atm.Logic.Common
{
  public static class DomainEvents
  {
    ...
    {
      foreach (Type handlerType in _staticHandlers)
      {
        bool canHandleEvent = handlerType.GetInterfaces()
          .Any(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(IHandler<>) && x.GenericTypeArguments[0] == domainEvent.GetType());
